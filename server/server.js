...
`;
  ...
  while (attempts < maxAttempts) {
    try {
      console.log(`🔄 Reasoning attempt ${attempts + 1} with model: ${currentModelKey}`);
      
      // احصل على التكوين للنموذج الحالي
      const reasoningConfig = MODEL_CONFIGS[currentModelKey];
      if (!reasoningConfig) {
        console.error(`❌ Model config not found: ${currentModelKey}`);
        currentModelKey = getFallbackModel(currentModelKey);
        attempts++;
        continue;
      }

      // احصل على المفتاح للنموذج الحالي
      keyInfo = getSafeKeyForModel(currentModelKey);
      if (!keyInfo) {
        console.log(`❌ No available keys for ${currentModelKey}, trying fallback...`);
        currentModelKey = getFallbackModel(currentModelKey);
        attempts++;
        continue;
      }

      if (reasoningConfig.provider === "google") {
        const genAI = new GoogleGenerativeAI(keyInfo.token);
        const model = genAI.getGenerativeModel({
          model: reasoningConfig.modelName,
          generationConfig: {
            temperature: 0.2,
            maxOutputTokens: 600
          }
      ...
      } else if (reasoningConfig.provider === "openrouter") {
        console.log(`🔑 Using OpenRouter model: ${reasoningConfig.modelName} with key: ${keyInfo.id}`);
        ...
        if (!response.ok) {
          const errorText = await response.text();
          console.log(`⚠️ OpenRouter response: ${response.status} - ${errorText.substring(0, 200)}`);
          
          if (response.status === 429) {
            console.log(`📉 Model ${currentModelKey} rate limited (429), trying fallback...`);
            currentModelKey = getFallbackModel(currentModelKey);
            attempts++;
      ...
      if (error.message === 'RATE_LIMITED' || error.message.includes('429') || error.message.includes('rate limit')) {
        currentModelKey = getFallbackModel(currentModelKey);
        attempts++;
        
        // إذا وصلنا لمحاولات كثيرة جداً، نستخدم Gemini Flash كملاذ أخير
        if (attempts >= maxAttempts) {
          console.log(`⚠️ All reasoning models failed, using Gemini Flash as final fallback`);
          
          // استخدام Gemini Flash مباشرة
          const geminiKey = getSafeKey('gemini');
          if (geminiKey) {
            const genAI = new GoogleGenerativeAI(geminiKey.token);
            const model = genAI.getGenerativeModel({
              model: "gemini-3-flash-preview",
              generationConfig: {
                temperature: 0.2,
...
const combinedModelName = `${reasoningModelName} + ${modelConfig.displayName}`;
