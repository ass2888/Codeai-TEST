...
function processFilesUpdate(fullText) {
    ...
    // Regex للبحث عن التاجات
    const blockRegex = /<(FILE|DIFF)\s+(?:name|file)="([^"]+)"\s*>([\s\S]*?)<\/\1>/gi;
    ...
    while ((match = blockRegex.exec(fullText)) !== null) {
        ...
        // --- إضافة طباعة للكونسول للتتبع ---
        console.log(`🔎 Found Block: [${type}] for file: ${fileName}`);
        ...
        if (type === 'FILE') {
            ...
            if (targetFile) {
                try {
                    console.log(`🔧 Applying diff to: ${fileName}`);
                    // طباعة المحتوى الكامل للـ DIFF لنرى ماذا أرسل النموذج
                    console.log("RAW DIFF:", content); 

                    const newContent = applyPatch(targetFile.content, content);
                    ...
                    console.error("❌ Diff Patch Failed:", e);
                }
            } else {
                console.warn(`⚠️ File not found for diff: ${fileName}`);
            }
        }
...
// 4) استقبال النص
    ...
    /* --- قم بتعليق هذا الكود المسبب للإخفاء ---
...
// --- Version Display Logic ---
    const APP_VERSION = 'v1.275.3'; // يمكنك تحديث هذا يدوياً عند كل تحديث للكاش
